...
#include <vector>
#include <cstdint>
#include <regex>//áèáëèîòåêà äëÿ ðàáîòû ñ ðåãóëÿðíûìè âûðàæåíèÿìè
#include <algorithm>
#include <string>
...
int main(int argc, char* argv[])
{
   ...
   if (argc < 2)
    ...
    while (!input_file.eof())
    {
        ...
        if (input_byte == '\n')
        {
            if (!code_line.empty() && code_line[0] != 10 && code_line[0] != 13)//êîäû ïåðåâîä ñòðîêè è êîíåö ñòðîêè
            {
...
{
    string input_str;
    regex pattern_value("[a-zA-Z0-9]+");//øàáëîí äëÿ ïðîâåðêè êîððåêòíîñòè ââîäà ñèìâîëîâ â îïåðàöèè read
